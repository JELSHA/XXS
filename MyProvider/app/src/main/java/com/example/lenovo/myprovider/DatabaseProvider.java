...
public class DatabaseProvider extends ContentProvider {
    ...
    public String getType(Uri uri) {
        switch (uriMatcher.match(uri)){
            ...
            case CONTACTS_ITEM:
                return "vnd.android.cursor.item/vnd.com.axample.databasetest.provider.contacts";
        }
        return null;
    }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (uriMatcher.match(uri)){
            case CONTACTS_DIR:
            case CONTACTS_ITEM:
                long newName = db.insert("contacts",null,values);
                uriReturn = Uri.parse("content://"+AUTHORITY+"/contacts/"+newName);
                break;
            default:break;
        }
        return uriReturn;
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        Cursor cursor = null;
        switch (uriMatcher.match(uri)){
            case CONTACTS_DIR:cursor = db.query("contacts",projection,selection,selectionArgs,null,null,sortOrder);
            break;
            case CONTACTS_ITEM:
                String Name = uri.getPathSegments().get(1);
    ...
    public int update(Uri uri, ContentValues values, String selection,
       ...
       switch (uriMatcher.match(uri)){
           ...
           case CONTACTS_ITEM:
               String Name = uri.getPathSegments().get(1);
               updateRows = db.update("contacts",values,"name = ?",new String[]{Name});
               break;
           default:break;
       }
       return updateRows;
